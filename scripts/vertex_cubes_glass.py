#!/usr/bin/env python3
"""Generate the vertex cube field with a glass sphere in front of it.
...
A glass sphere sits on the line from the camera to the centre cube, close enough
to the plane to stay near the focal distance. A sphere of glass is a lens, so
the middle of the frame is the field seen through it: inverted, and shrunk to
fit inside the sphere's outline.
...
# The glass sphere, placed this far off the plane along the line to the camera.
# Its size is given in the frame rather than the world: SCREEN_RADIUS is how
# much of the canvas width its radius covers, and the world radius follows from
# how far the sphere ends up from the camera.
GLASS_OFFSET = 7.0
GLASS_SCREEN_RADIUS = 0.25
GLASS_IOR = 1.5
...
def glass():
    centre = scale(unit(POSITION), GLASS_OFFSET)
    depth = DISTANCE - GLASS_OFFSET
    # Half the canvas width subtends this much tangent; the silhouette's edge has
    # to land at the requested fraction of it.
    half_width = TAN_HALF_FOV * ASPECT
    radius = depth * math.sin(math.atan(half_width * 2 * GLASS_SCREEN_RADIUS))
    return (
        f'[materials.glass]\ntype = "dielectric"\nior = {GLASS_IOR:.4g}\n\n'
        f'[[objects]]\nname = "glass sphere"\ntype = "sphere"\nmaterial = "glass"\n'
        f"center = {vec(centre)}\nradius = {radius:.4g}\n"

...
(function() {
    ...
    function updateTicks(chartId, minTime, maxTime) {
        ...
        ticks.forEach(tick => {
            const y = plotY + plotHeight - ((tick - minTime) / timeRange) * plotHeight;
            ...
            const labelText = (tick === Math.floor(tick)) ? tick.toString() : tick.toFixed(1);
        ...
        if (selectionRect) {
            ...
            if (width > 5 && height > 5) {
                ...
                if (linesGroup) {
                    ...
                    updateChart(currentChartId, newMinT, newMaxT, newMinTime, newMaxTime, true);
                }
            }
        }
